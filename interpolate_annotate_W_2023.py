#--------------------------------------------------------- 
# Interpolate and annotate vertical velocity (2023)
# Tom Carrard
# 05.08.24
#---------------------------------------------------------
...
traj_file  = 'GPS_allsoaringpoints_areavol_subset_2023.csv'

df_traj_all = pd.read_csv(traj_path+traj_file)

df_traj_all['timestamp'] = pd.to_datetime(df_traj_all['timestamp'])

df_traj_all['height.above.sea.level'].max()

#------------------------------------------------------
# Subset for RAM limitation
#------------------------------------------------------
...
event = df_traj_all['burstID'].iloc[100]
#%% 
#--------------------------------------------------------
...
def interp_at_eagle(event,event_class=event_class):
    ...
    eagle_outdata['z']         = z_interp #height of model levels interpolated at eagle location
    ...
    if (eagle_outdata['height.above.surf'] < 0).any():
        print('problem with model level interpolation') 
        
    #print('Successful interpolation for ', event)

    #why not the same for eagle_outdata and df_subset ???
...
#do the interpolation, then assign NAs to problematic points
...
#
#df0 =pd.read_csv('/home/tcarrard/eagle.gravity.waves/data/GPS_allsoaringpoints_areavol_subset_2020.csv')
#
...
#import pandas as pd
#import numpy as np
df_2023 = pd.read_csv(traj_path+traj_file[:-4]+'_annotated_W.csv')
#
#df_2020 = pd.read_csv('/home/tcarrard/eagle.gravity.waves/data/GPS_allsoaringpoints_areavol_subset_2020_annotated_W.csv')
#
#df_all = pd.concat([df_2020,df_2023])
#
#df_all.iloc[:,3:].to_csv('/home/tcarrard/eagle.gravity.waves/data/GPS_allsoaringpoints_areavol_subset_annotated_W.csv')
#
df_all_old = pd.read_csv('/net/thermo/atmosdyn/tcarrard/data/eagles/data/GPS_allsoaringpoints_areavol_subset_annotated_W.csv')
#
#len(df_all)
#
#df_all['timestamp'] = pd.to_datetime(df_all['timestamp'])
#
#df_all[(df_all['W'].isna()) & (df_all['height.above.surf']>20) & (df_all['timestamp'].dt.year==2023)]['burstID']
#
#df_all[(~df_all['W'].isna()) & (df_all['height.above.surf']<0) & (df_all['timestamp'].dt.year==2023)]['burstID']


df_new = pd.read_csv(traj_path+traj_file[:-4]+'_annotated_fullwind.csv')

df2020 = pd.read_csv('/net/thermo/atmosdyn/tcarrard/data/eagles/data/GPS_allsoaringpoints_areavol_subset_2020_annotated_fullwind.csv')

df_all = pd.concat([df2020,df_new])

df_all.iloc[:,3:].to_csv('/net/thermo/atmosdyn/tcarrard/data/eagles/data/GPS_allsoaringpoints_areavol_subset_annotated_fullwind.csv')

df_2023['W'].isna().sum()
df_new['W'].isna().sum()

df_new.groupby('interp').count()

import matplotlib.pyplot as plt


plt.scatter(df_all['W'],df_all_old['W'])
plt.xlabel('Old')
plt.ylabel('New')